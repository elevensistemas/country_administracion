...
@section('title', 'Nueva Unidad Funcional')
@section('page_title', 'Crear Unidad Funcional')

@section('content')
<div class="row justify-content-center">
    <div class="col-lg-8">
        <form method="POST" action="{{ route('admin.functional-units.store') }}">
            @csrf

            <!-- General Functional Unit Data -->
            <div class="ios-card">
                <h5 class="fw-bold mb-4"><i class="bi bi-building-fill text-success me-2"></i>Datos de la Unidad Funcional</h5>
                ...
                <div class="row g-3">
                    <div class="col-md-6">
                        ...
                        <select name="lot_id" id="lot_id" class="form-select form-control-ios @error('lot_id') is-invalid @enderror" required>
                            <option value="">Selecciona el lote correspondiente</option>
                            @foreach($lots as $lot)
                    ...
                    <div class="col-md-6">
                        ...
                        <input type="text" name="code" id="code" class="form-control form-control-ios @error('code') is-invalid @enderror" value="{{ old('code') }}" required placeholder="Ej: UF-LOT-012">
                        @error('code')
                            <span class="invalid-feedback">{{ $message }}</span>
                        @enderror
                    </div>
                    ...
                    <div class="col-md-6">
                        ...
                        <input type="text" name="name" id="name" class="form-control form-control-ios @error('name') is-invalid @enderror" value="{{ old('name') }}" required placeholder="Ej: UF Lote 12">
                        @error('name')
                            <span class="invalid-feedback">{{ $message }}</span>
                        @enderror
                    </div>
                    ...
                    <div class="col-12">
                        ...
                        <input type="text" name="description" id="description" class="form-control form-control-ios @error('description') is-invalid @enderror" value="{{ old('description') }}" placeholder="Detalles de la unidad...">
                        @error('description')
                            <span class="invalid-feedback">{{ $message }}</span>
                        @enderror
                    </div>
            ...
            <div class="ios-card">
                ...
                <p class="text-muted mb-4" style="font-size: 0.85rem;">Asigna los propietarios responsables del pago de expensas de esta unidad funcional.</p>
                            ...
                            @foreach($owners as $owner)
                                <div class="col-sm-6 col-md-4 mb-2">
                                    <div class="form-check">
                                        <input class="form-check-input text-success" type="checkbox" name="owner_ids[]" value="{{ $owner->id }}" id="owner_{{ $owner->id }}" {{ is_array(old('owner_ids')) && in_array($owner->id, old('owner_ids')) ? 'checked' : '' }}>
            ...
            <div class="d-flex justify-content-between align-items-center mb-5">
                ...
                <button type="submit" class="btn btn-ios btn-ios-primary px-4"><i class="bi bi-check-lg me-2"></i>Crear Unidad Funcional</button>
            </div>
        </form>
    </div>
</div>
